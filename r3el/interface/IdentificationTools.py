"""Discover and invoke the identification tool through a stdio MCP subprocess."""
...
class IdentificationTools:
    ...
    async def __aenter__(self):
        ...
        try:
            ...
            tool = next(tool for tool in tools.tools if tool.name == 'submit_identification')
    ...
    async def submit(self, arguments: dict) -> dict:
        result = await self._client.call_tool('submit_identification', arguments)
        if result.is_error:
