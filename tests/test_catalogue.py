...
class CatalogueMatchingTests(unittest.TestCase):
    ...
    def test_manual_id_uses_details_and_catalogues_without_search_or_llm(self):
    ...
    def test_invalid_manual_id_keeps_multiple_matches_for_correction(self):
    ...
    def test_manual_id_rejects_active_or_already_resolved_rows(self):
        ...
        self.item.tmdb_match = TMDBMatch('Movie', 2020,
            response={'total_results': 1, 'results': [{'id': 42}]})
        with self.assertRaises(WorkspaceActionConflict):
            self.runner.match_id('batch', 'file', 42)
        self.batch.state = MediaFileBatchState.MATCHING
        with self.assertRaises(WorkspaceActionConflict):
            self.runner.match_id('batch', 'file', 42)
