...
class ControlServerTests(unittest.TestCase):
    ...
    def test_catalogue_random_search(self, random, recent, categories):
        ...
        random.return_value = []
        self.assertIn('No shows, episodes or movies', self.request('/catalogue?random=1')[2])
        for query in ('random=bad', 'random=', 'random=1&random=1'):
            self.assertEqual(self.request('/catalogue?' + query)[0], 400)
    ...
    def test_catalogue_recent_and_title_search(self, categories, movies, recent):
        ...
        self.assertIn('TV Series: 24</h2>', body)
        self.assertIn('TV Episodes: 1,234</h2>', body)
    ...
    def test_dropdowns_follow_selected_branch(self):
        for query, expected_subcategories, expected_events in (
            ...
            ('category=TMDB', {'Search', 'Result', 'Details'}, {'tmdb_search', 'tmdb_result', 'tmdb_details'}),
            ('category=TMDB&subcategory=Search', {'Search', 'Result', 'Details'}, {'tmdb_search'}),
            ('category=TMDB&subcategory=Details', {'Search', 'Result', 'Details'}, {'tmdb_details'}),
            ('category=TMDB&subcategory=Result', {'Search', 'Result', 'Details'}, {'tmdb_result'}),
            ...
            ('subcategory=Lifecycle',
             {'Lifecycle', 'Discovery', 'BatchIdentification', 'ToolConversation', 'SubmissionHandler', 'LLMPrompt', 'Search', 'Result', 'Details', 'Move', 'Delete', 'Download', 'Create Record'},
